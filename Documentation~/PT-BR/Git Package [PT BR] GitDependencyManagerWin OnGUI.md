# Git Package [PT BR] GitDependencyManagerWin OnGUI
## Campos&Propriedades
```c#
...
```
## OnEnable()
O metódo inicia e define alguns processos.
```c#
...
```
## OnDestroy()
Quando a janela de gerenciamento é fechado a lista de manifestos são descarregados para seus respectivos
arquivos .gpack.(exceto os manifestos externos)
```c#
...
```
## Refresh()
Metódo responsavel por atualizar a lista de manifestos.
```c#
...
```
## SearchRequestUpdate()
Esse metódo verifica quais manifestos são externos ou não.
```c#
